        ...
        NavigationStack {
            ZStack {
                ...
                VStack {
                    HStack {
                        Spacer()
                        Button(action: { viewModel.generateMaze() }) {
                            Image("Restart")
                                .resizable()
                                .frame(width: DeviceUtils.isPhone(sizeClass: sizeClass) ? 32 : 48, height: DeviceUtils.isPhone(sizeClass: sizeClass) ? 32 : 48)
                                .padding()
                        }
                        Spacer()
                    }
                    ...
                    HStack {
                        Button(action: { viewModel.movePlayer(direction: .left) }) {
                            Image("Arrow")
                                .resizable()
                                .frame(width: DeviceUtils.isPhone(sizeClass: sizeClass) ? 80 : 120, height: DeviceUtils.isPhone(sizeClass: sizeClass) ? 80 : 120)
